...
def api_request(query, app, variables=None, retries=3):
    ...
    """
    log("Function api_request called.")
    for _ in range(retries):
        ...
        log("Sent the POST request.")

        rate_limit_remaining = int(response.headers.get("X-RateLimit-Remaining", 0))
        rate_limit_reset = int(response.headers.get("X-RateLimit-Reset", 0))
        log(f"Got the rate limit headers: {rate_limit_remaining}, {rate_limit_reset}.")

        if response.status_code == 429:
            log("The status code is 429. The rate limit has been hit.")
            ...
            if wait_time < 0:
                ...
                log("The rate limit reset time is in the past.")
                wait_time = 65
                app.update_terminal(f"Waiting for {wait_time} seconds.\n")
                log(f"Set the wait time to: {wait_time} seconds.")
        ...
        if response.status_code == 200:
            log("The status code is 200. The request was successful.")
        ...
        if response.status_code == 500:
            ...
            log("The status code is 500. Server error, retrying request.")
            time.sleep(2)
            continue
...
def Set_Access_Token(app):
    ...
    """
    global headers
    log("Function Set_Access_Token called.")
    config = load_config("config.json")
    log("Loaded the configuration from config.json.")
    try:
        if config["ACCESS_TOKEN"] is not None:
            log("Access token found in the configuration.")
            ...
            log("Defined the headers for the API request.")
        else:
            log("No access token found in the configuration.")
...
def needs_refresh(app):
    ...
    """
    log("Function needs_refresh called.")
    # Define a simple query
    query = Queries.VIEWER
    log("Defined the query.")
    try:
        ...
        log("Error: Cannot resolve graphql.anilist.co")
