...
export function mapAniListToKenmeiStatus(
  ...
  Object.entries(DEFAULT_STATUS_MAPPING).forEach(([kenmeiStatus, anilistStatus]) => {
  ...
  if (customMapping) {
    Object.entries(customMapping).forEach(([kenmeiStatus, anilistStatus]) => {
      ...
      [...reverseMapping.entries()].forEach(([key, value]) => {
...
function validateAniListStatus(status: string): MediaListStatus | undefined {
  ...
  // Map common variations
  if (['PLAN', 'PLANNING_TO_READ', 'PTR', 'PTW'].includes(normalized)) return 'PLANNING';
  if (['WATCHING', 'READING', 'CURRENT'].includes(normalized)) return 'CURRENT';
  if (['DONE', 'COMPLETE', 'FINISHED', 'COMPLETED'].includes(normalized)) return 'COMPLETED';
  if (['DROP', 'QUIT', 'DROPPED'].includes(normalized)) return 'DROPPED';
  if (['HOLD', 'ON_HOLD', 'PAUSED'].includes(normalized)) return 'PAUSED';
  if (['REPEAT', 'REREADING', 'REWATCHING', 'REPEATING'].includes(normalized)) return 'REPEATING';
 ...
 */
export function getAllPossibleStatusMappings(): Record<KenmeiStatus, MediaListStatus[]> {
  return {
